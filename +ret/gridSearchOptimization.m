...
if bounds.pinDia(2) > bounds.pinDia(1), pinDia_vals = linspace(bounds.pinDia(1), bounds.pinDia(2), 6); else, pinDia_vals = bounds.pinDia(1); end

bestObj = inf;
bestParams = [];

totalCombinations = numel(t_vals)*numel(nRows_vals)*numel(nPinsPerRow_vals)*numel(rowSpacing_vals)*numel(firstRowZ_vals)*numel(pinDia_vals);
